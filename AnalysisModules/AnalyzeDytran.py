...
def lowPass(dytran):
	newXPoints = [x for x in range(1000,700001,1000)] #Array of new x axis points to be used, does not start at 0 to avoid 0,0 point
...
def LinearPiecewise(x,x0,y0,m1,m2): #x = x data, x0 and y0 are the coordinates of the 'turning point', m1 and m2 are the slopes of the 1st and second segments of the line, respectively
...
def Fit(xdata,ydata): #Input the x and y data that needs to be fitted, !!!Make sure x,y are np.arrays, not just lists
	params, covariance = optimize.curve_fit(LinearPiecewise,xdata,ydata) #Returns the values of the parameters that best fit the data. Also returns covariance, but that's not important
	return params
