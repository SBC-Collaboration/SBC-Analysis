...
for nsim in neutron_sims:
    nsim['hiddenvars'] = np.random.rand(n_hiddenvars, nsim['Er'].size)
...
def SimulatedCounts(xpts, eb, i_exp):
    ...
    bubbles = np.array(neutron_sims[i_exp]['hiddenvars'][1:, :] < p[:, np.newaxis, :], dtype=np.uint8)
    ...
    bubcount = np.zeros((bubbles.shape[0],
                         ...
                         bubbles.shape[2] + 1),
                        dtype=np.uint8)
    bubcount[:, :, 1:] = np.cumsum(bubbles, axis=2, dtype=np.uint8)
    nbub = np.diff(bubcount[:, :, ev_posts], axis=2)
    ...
    vetocount = np.zeros((bubbles.shape[0],
                          ...
                          bubbles.shape[2] + 1),
                         dtype=np.uint8)
    np.cumsum(vetoed_bubbles, axis=2, out=vetocount[:, :, 1:], dtype=np.uint8)
    vetoed = np.diff(vetocount[:, :, ev_posts], axis=2) > 0
    nbub[vetoed] = 0
    ...
    fidcount = np.zeros((bubbles.shape[0],
                         ...
                         bubbles.shape[2] + 1),
                        dtype=np.uint8)
    np.cumsum(fid_bubbles, axis=2, out=fidcount[:, :, 1:], dtype=np.uint8)
    infid = np.diff(fidcount[:, :, ev_posts], axis=2) > 0
    ...
    nbub_test = np.arange(1, bkg_counts.shape[1]+1, 1, dtype=np.uint8)
    # nbub_test.shape = (k,)
    ...
    trials_rescale = 1.0 / bubbles.shape[1]
...
# A division by zero safe version of PoissonChisq
def PoissonChisq(n, nu):
    ''' Calculates Poisson chi-square '''
    chisq_0 = np.float64(2 * (nu - n))
    c_n = n != 0
    chisq_0[c_n] += -2 * n[c_n] * np.log(nu[c_n] / n[c_n])
