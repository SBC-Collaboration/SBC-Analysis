...
def PICOcalLL(theta, whichnuisance=np.ones(n_nuisance, dtype=np.bool)):
    ...
    dEpts[0,0,:] = threshold_fenceposts[0] + np.exp(dEpts_reparam[0,0,:])
    dEpts[1:,0,:] = np.exp(dEpts_reparam[1:,0,:])
...
def PICOcalLL_post(theta, whichnuisance=np.ones(n_nuisance, dtype=np.bool)):
    ...
    dEpts[0,0,:] = threshold_fenceposts + np.exp( dEpts_reparam[0,0,:])
    dEpts[1:,0,:] = np.exp(dEpts_reparam[1:,0,:])
    ...
    if (total_chisq + np.sum(eb * eb)) > 62:
        return -np.inf
