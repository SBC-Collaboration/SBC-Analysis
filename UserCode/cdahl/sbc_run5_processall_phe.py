...
for runname in runlist:
    ...
    if (runid[0] >= 20161031) and\
        ...
        existing_phea = sbc.read_bin(os.path.join(recondir, runname,
                                                  'PMTpheAnalysis_' +
                                                  runname + '.bin'))
        if len(existing_phea['pmt_npeaks'].shape) == 1 or\
                existing_phea['pmt_npeaks'].shape[1] == 1:
            try:
                sbc.psr_pmtphea(os.path.join(datadir, runname),
