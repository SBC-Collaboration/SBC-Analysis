...
def AcousticAnalysis(ev, tau, piezo_fit_type=0,
    ...
    default_output = dict(bubble_t0=np.zeros((1, 2))+np.nan,  # seconds
                          peak_t0=np.zeros((1, 2))+np.nan,  # seconds
                          piezoE=np.zeros((1, 2,
                                           t_wins.shape[0],
                                           f_bins.shape[0] - 1),
                                          dtype=np.float64) + np.nan,
                          piezo_list=np.zeros((1,2))-1,
                          piezo_freq_binedges=np.array([f_bins]),
                          piezo_time_windows=np.array([t_wins]),
                          led_switch_off_amp=np.zeros((2, 2)),
                          led_tau=np.zeros((1, 2)),
