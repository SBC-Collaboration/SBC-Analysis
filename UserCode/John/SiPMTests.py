...
class SiPMPlotter(object):
    def __init__(self, pmt_data, default_trigger=0, runid=None, ev=None):
    ...
    def update_title(self):
        plt.suptitle("Runid: {} || Event: {} || Trigger {}\nUse Left and Right arrows to navigate."\
                     .format(self.runid, self.ev, self.trigger.trig))
...
if __name__ == "__main__":
    ...
    plt.hist(areas, bins=3600, fill=False, color=colors, histtype="step", stacked=False)
    plt.legend(labels)
    plt.suptitle("Areas")
    plt.figure()
    plt.hist(max_times, bins=3600, fill=False, color=colors, histtype="step", stacked=False)
    plt.legend(labels)
