...
def get_pulse(trace,t,dt,pk_loc,std):
    ...
    for i in range(len(t)):
        ...
        if trace[i]>std and np.abs(t[i]-pk_loc)<1e-7:
            tPulse.append(t[i])
...
def SBC_pulse_integrator_bressler(trace,dt):
    ...
    totIntegral = scipy.integrate.trapz(trace,tPMT)*CONVERSION_TO_CHARGE
    if Npeaks == 1:
...
def main():
    ...
    for event in events:
        ...
        for i in range(len(trac)):
            ...
            if NTrig:
                [a_trig,n_trig,totInt] = SBC_pulse_integrator_bressler(trace,dt_tr)
                if n_trig == 0:
                    number = 'zero'
                elif n_trig == 1:
                    number = 'one'
                ...
                [a,n,totInt] = SBC_pulse_integrator_bressler(trace,dt_tr)
                if n == 0:
                    number = 'zero'
                elif n == 1:
                    number = 'one'
